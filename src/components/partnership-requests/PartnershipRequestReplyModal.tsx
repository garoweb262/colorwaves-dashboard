...
export function PartnershipRequestReplyModal({ request, isOpen, onClose, onReply }: PartnershipRequestReplyModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg" title="Reply to Partnership Request">
      <div className="p-6 h-[80vh] overflow-y-auto scrollbar-hide">
        {request && (
          <div className="space-y-6">
            {/* Original Request */}
            <div className="glass-form-section p-4">
              <h3 className="font-medium text-white mb-3">Partnership Request Details</h3>
              <div className="space-y-2 text-sm">
                <p className="text-white/80"><strong className="text-white">Contact:</strong> {request.fullName} ({request.email})</p>
                <p className="text-white/80"><strong className="text-white">Company:</strong> {request.companyName}</p>
                <p className="text-white/80"><strong className="text-white">Partnership Type:</strong> {request.partnershipType}</p>
                ...
                <div className="glass-card p-3 rounded border-l-4 border-purple-500/50">
                  <p className="whitespace-pre-wrap text-white/90">{request.description}</p>
                </div>
              </div>
            </div>
